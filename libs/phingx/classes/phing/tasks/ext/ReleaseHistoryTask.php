...
class ReleaseHistoryTask extends Task
{
    ...
    public function main()
    {
        ...
        if ( !$type )
        {
            throw new BuildExcetion( 'Не указан параметр type для таргета release_history' );
        }
    ...
    protected function _runItem( SimpleXMLElement $item )
    {
        ...
        if ( method_exists( $this, $method ) )
        {
            ...
            $this->log(
                'Save|notify ' . ( $res ? 'completed successful' : 'failed' ),
                ( $res ? PROJECT::MSG_INFO : PROJECT::MSG_WARN ) );
        }
        else
        {
    ...
    protected function _mailByUrl( SimpleXMLElement $config )
    {
        ...
        if ( $url && $projectId && $this->_bt )
        {
            ...
            $url = sprintf( '%s%sproject=%s&action=%s&user=%s', $url,
                $delimiter, $projectId, $this->_bt, $user );
    ...
    protected function _local( SimpleXMLElement $el )
    {
        ...
        $h = new ReleaseHistory( $this->_config, $this->getProject() );
        $res = $h->deleteOldItems();

        foreach ( $res as $libName => $resultLib )
        {
            if ( !empty( $resultLib ) )
            {
                $this->log(
                    sprintf( 'For "%s" deleted: "%s"', $libName,
                        implode( ', ', $resultLib ) ) );
            }
        }
        $this->log( 'Done.' );
